...
<head>
    ...
    <title><?= __e('page.success.title') ?> - <?= htmlspecialchars($certData['event_name']) ?> - <?= __e('site.name') ?></title>
...
<body>
    ...
    <div class="main-wrapper">
        ...
        <div class="container">
            <h1><?= __e('page.success.title') ?></h1>
            <p><?= __e('page.success.subtitle') ?></p>

           <a href="download.php?id=<?= htmlspecialchars($certId) ?>" id="downloadBtn" class="btn-primary">
                <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/></svg>
                <?= __e('page.success.download-btn') ?>
            </a>

            <hr style="border: 0; border-top: 1px solid var(--border-color); margin: 20px 0;">

            <h3 style="margin-top: 0; color: #0f172a; font-size: 18px; font-weight: 700;"><?= __e('page.success.add-profile') ?></h3>
            <p style="font-size: 14px; margin-bottom: 24px; text-align: left;"><?= __e('page.success.add-profile-desc') ?></p>
            ...
            <a href="<?= $linkedInShareDesktop ?>" target="_blank" class="btn-linkedin-outline" onclick="return handleLinkedInShare(event, '<?= $linkedInShareMobile ?>');">
                <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M4.98 3.5c0 1.381-1.11 2.5-2.48 2.5s-2.48-1.119-2.48-2.5c0-1.38 1.11-2.5 2.48-2.5s2.48 1.12 2.48 2.5zm.02 4.5h-5v16h5v-16zm7.982 0h-4.968v16h4.969v-8.399c0-4.67 6.029-5.052 6.029 0v8.399h4.988v-10.131c0-7.88-8.922-7.593-11.018-3.714v-2.155z"/></svg>
                <?= __e('page.success.post-linkedin') ?>
            </a>

            <div class="form-group" style="margin-top: 20px;">
                <label><?= __e('page.verify.label.cert-id') ?></label>
            ...
            <div class="form-group">
                <label><?= __e('page.success.verify-link') ?></label>
                <div class="input-group">
